...
*/
    ...
    private function checkInput($input, $key, $type) {
        ...
        if ($type !== 'HEADER' && $this->config['general']['check_lfi'] && 
            ...
            $this->addThreat("LFI", "$type parameter '$key': $decoded");
            $this->blockRequest("LFI detected in $type parameter '$key': $decoded");
            return;
        }
    ...
    private function checkSSRF($input, $key, $type) {
        ...
        if ($type === 'HEADER' && $key === 'Origin') {
            if (strpos($input, 'localhost') !== false || strpos($input, '127.0.0.1') !== false) {
                return false; // 不触发 SSRF 警报
            }
        }
    ...
    private function checkUpload() {
        foreach ($_FILES as $file) {
            if (isset($file['error']) && $file['error'] == 0) {
                ...
                if ($this->matchPatterns($filename, array($blackExt)) || !$this->matchPatterns($filename, array($whitelist))) {
